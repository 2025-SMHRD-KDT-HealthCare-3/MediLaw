...
class Chat(Base):
    ...
    # AI 답변은 chatter_id를 NULL로 저장하거나 시스템 사용자 ID를 사용할 수 있다.
    chatter_id: Mapped[int | None] = mapped_column(ForeignKey("tb_user.user_id"), nullable=True)
    # speaker_type은 USER, AI, ADMIN 발화자를 구분한다.
    ...
    chatted_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now, nullable=False)
