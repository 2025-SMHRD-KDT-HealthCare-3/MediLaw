...
SYSTEM_PROMPT = (
    ...
    "'- '(하이픈+공백)로 시작하는 불릿으로 정리합니다. 불릿은 반드시 한 줄에 하나씩 쓰고, "
    "연속된 불릿 사이에도 반드시 빈 줄 하나를 넣으세요. 각 항목은 짧게 요약하고 끝에 "
    "인용번호[1]를 붙입니다.\n"
    "   - 인용을 '[1] 내용 [2] 내용' 형태로 한 줄에 이어 쓰지 마세요. 인용 하나당 반드시 별도의 "
    "'- ' 불릿 한 줄로 나누어 쓰세요.\n"
...
SYSTEM_PROMPT_EN = (
    ...
    "statutes/precedents as bullet points starting with '- ', each ending with its citation "
    "number [1]. Each bullet MUST be on its own line, with one blank line between consecutive "
    "bullets.\n"
    "   - Never run citations together on one line like '[1] text [2] text'; put each citation "
    "in its own '- ' bullet line.\n"
